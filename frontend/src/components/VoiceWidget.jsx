...
const API_BASE_CANDIDATES = Array.from(
  new Set([import.meta.env.VITE_API_BASE_URL, "http://localhost:8000", "http://localhost:8001"].filter(Boolean)),
);
...
async function getApi(path) {
  ...
  for (const base of API_BASE_CANDIDATES) {
    try {
      return await axios.get(`${base}${path}`, { timeout: 15000 });
    } catch (error) {
      lastError = error;
...
export function VoiceWidget() {
  ...
  useEffect(() => {
    ...
    const checkConnection = async () => {
      ...
      try {
        ...
        const response = await getApi(`/api/voice-agent/status`);
  ...
  const handlePhoneCall = async () => {
    ...
    try {
      ...
      const response = await postApi(`/api/voice-agent/call`, {
