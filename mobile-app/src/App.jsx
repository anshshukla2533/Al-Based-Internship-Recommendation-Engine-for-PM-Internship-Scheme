...
function App() {
  ...
  const selectLanguage = async (choice) => {
    ...
    if (langObj.lang === "en") return setUi(defaultUIStrings);

    setTranslating(true);

    try {

      const res = await axios.post(`${API_BASE}/translate`, { target_language: langObj.lang, payload: defaultUIStrings });

      setUi(res.data.translated_payload || defaultUIStrings);

    } catch { setUi(defaultUIStrings); }
  ...
  const handleManualSubmit = async (selectedSkills) => {
    ...
    } catch (e) { alert(e?.response?.data?.detail || "Backend not reachable."); }
  ...
  const handleFileUpload = async (file) => {
    ...
    formData.append("location", profileDict.location || "Amaravati");
    ...
    } catch (e) { alert(e?.response?.data?.detail || "Network error."); }
  ...
  const fetchRecommendedJobs = async () => {
    ...
    try {
      ...
      const res = await axios.post(`${API_BASE}/recommended-jobs`, { ...profileDict, skills, target_language: language, lang: activeLang });

      setJobs(res.data.top_matches || []);

      navTo("results");

    } catch (e) { alert(e?.response?.data?.detail || "Error finding jobs."); }
  ...
  const startInterview = async (job) => {
    ...
    try {
      ...
      const res = await axios.post(`${API_BASE}/interview-prep`, { job_title: job.title, company: job.company, skills: interviewSkills });
    ...
    } catch (e) { alert(e?.response?.data?.detail || "Failed to pull interview questions."); }
  ...
  const ResultsScreen = () => (
    ...
    <motion.div {...pt} className="w-full pb-20 pt-4">
        ...
        {jobs.length === 0 ? <div className="p-8 border border-white/10 text-center font-pixel text-[10px] text-white/40 tracking-widest uppercase">{ui.noMatch}</div> : jobs.map((job, idx) => {
          ...
          return (
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1 }} key={job.id} className="bg-black/40 backdrop-blur-md border border-white/20 p-6 relative overflow-hidden">
              ...
              <p className="text-[8px] text-white/30 uppercase tracking-[0.25em] mb-3">Live on {sourceLabel}</p>

              <p className="text-xs text-white/70 leading-relaxed font-light border-l border-white/20 pl-3">{job.description}</p>

              {job.field && <p className="text-[10px] text-white/35 mt-3 uppercase tracking-[0.12em]">Field: {job.field}</p>}

              {visibleSkills.length > 0 && <div className="flex flex-wrap gap-2 mt-4">{visibleSkills.map(sk => (<span key={sk} className="text-[8px] uppercase tracking-wider px-2.5 py-1 border border-white/10 text-white/40 bg-white/5 font-bold">{sk}</span>))}</div>}
              ...
              <div className="mt-6 flex flex-col gap-3">
                ...
                <a href={job.apply_url || "https://pminternship.mca.gov.in/"} target="_blank" rel="noopener noreferrer" className="text-center bg-neon text-black tracking-[0.15em] font-black uppercase py-4 border border-neon btn-glow text-xs flex items-center justify-center gap-2">🌐 APPLY ON {sourceLabel.toUpperCase()} →</a>
      ...
      <div className="mt-8 space-y-6 pt-4 border-t border-white/10">
         ...
         <div className="bg-black/40 backdrop-blur-xl border border-white/20 p-6 shadow-2xl"><h3 className="font-pixel text-[8px] text-white/50 tracking-[0.3em] uppercase mb-4">TRAINING PATH</h3><LeetcodeHub company={targetCompany} jobTitle={targetJob?.title || "PM Internship"} skills={targetJob?.matched_skills?.length ? targetJob.matched_skills : (targetJob?.skills || skills)} /></div>
